...
const PRIMARY_BATCHES = ["batch1", "batch2"] as const;
const COMPARISON_BATCHES = ["batch3", "batch4", "batch5_cs"] as const;
...
function extractOfficeHoursHints(lines: string[]) {
...
function countExpectedOfficeHourDays(hints: string[]) {
  const patterns = [
    /\bmon(?:day)?s?\b/gi,
    /\btue(?:s(?:day)?)?s?\b/gi,
    /\bwed(?:nesday)?s?\b/gi,
