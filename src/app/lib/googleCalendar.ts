...
const GOOGLE_CALENDAR_SCOPES = "https://www.googleapis.com/auth/calendar.app.created";
...
export async function exportEventsToGoogleCalendar(
  ...
  const bucketKeys = narrowCalendarMode === "many"
  ...
  const calendarIdsByGroup = narrowCalendarMode === "many"
  ...
  const singleCalendarId =
    narrowCalendarMode === "many" ? "" : await createSingleExportCalendar(accessToken, courses);

  if (narrowCalendarMode !== "many") {
    completedSteps += 1;
  ...
  await runWithConcurrency(items, GOOGLE_EXPORT_CONCURRENCY, async ({ course, selection, event }) => {
    const colorId = googleEventColorIdForEvent(event, exportConfig);
    const calendarId = narrowCalendarMode === "many"
  ...
  return {
    ...
    calendarCount: narrowCalendarMode === "many" ? calendarIdsByGroup.size : 1,
  } satisfies GoogleCalendarExportResult;
}
